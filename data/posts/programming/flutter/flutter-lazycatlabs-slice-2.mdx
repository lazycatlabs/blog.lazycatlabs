...
images: ['/static/images/flutter/flutter_lazycatlabs_slice_2.png']
draft: false
...
And the next, we need to create `Offset Animation`, because we need to move the offset background when normal and on hover.
...
So, we create `Animation` variable will return `Offset` with start offset is `Offset.zero` and the end of animation to `Offset(0.1,0)` and we will use `CurvedAnimation` with parent from `_animationController` and use `curve: Curves.easeIn`, and also we need to refresh the page if animation is changed.
