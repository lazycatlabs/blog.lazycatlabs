...
In some cases, we need to hide some credential keys on our Application like `CLIENT_ID`, and `CLIENT_SECRET` even for your `GOOGLE_MAP_API` key. In my previous post talking about [Flutter: Store data based on Environment](https://blog.lazycatlabs.com/posts/programming/flutter/flutter-store-data-environment), we only hide the key from the `Dart Code`.
